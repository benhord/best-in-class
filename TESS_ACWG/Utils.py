...
def getBestInClassColor( i ):
    ...
    if i==1: # 'Green' = confirmed, has >5-sig mass measurement
        ...
        #backgroundColor = np.array( [225,207,130] )/256.
    elif i==3: # 'Orange' = unconfirmed, mass estimated, passes Kempton threshold
        #backgroundColor = np.array( [253,191,111] )/256.
        #backgroundColor = np.array( [203,70,200] )/256.
        backgroundColor = np.array( [120, 226, 256] )/256.
    elif i==4: # 'Red' = unconfirmed, mass estimated, does NOT pass Kempton threshold or false positive from this work
        backgroundColor = np.array( [251,154,153] )/256.
    elif i==5: # Blue = validated by this work
        backgroundColor = np.array( [50, 170, 250] )/256.
    elif i==6: # Gray = inconclusive validations
        #backgroundColor = np.array( [175, 175, 175] )/256.
        backgroundColor = np.array( [236, 236, 236] )/256.
    else:
        pdb.set_trace()
...
def writeToGridCell( ax, plNames, nwrite, y0, dy, ixs, pl, plTESS, TstarK, TOIGrid, SMFlag, \
                     plDict, SMVals, top5SM, ASCII, bestInClass, sMass, xtxt, xtxtR, text_fs, xsymb, confirmed ):
    """
    ...
    SMStrs = []
    TstarKs = []
    jwstObs = pd.read_csv('TESS_ACWG/obsJWST.csv', header=0)
    tsmObs = jwstObs[jwstObs['obsType'] == 'transit']
    esmObs = jwstObs[(jwstObs['obsType'] == 'eclipse') |
                     (jwstObs['obsType'] == 'phase')]
        ...
        for k in range( nwrite ):
            ...
            if plTESS != None:
                ...
                if plStrMatch in plTESS:
                    wt = 'normal'#'bold' #could change this to 'normal'
            if TOIGrid==True:
                wt = 'normal'
            # read in observed targs and bold observed JWST targs above
            if SMFlag == 'TSM' and plStr in tsmObs['name'].values:
                wt = 'normal'#'bold'
            elif SMFlag == 'ESM' and plStr in esmObs['name'].values:
                wt = 'normal'#'bold'
            if bestInClass==True:
                bc = getBestInClassColor( sMassOrdered[k] )
            elif confirmed==True:
                bc = np.array( [178,223,138] )/256.
            else:
                bc = 'none'
...
def legendBestInClass( fig ):
    bc = {}
    for i in [1,2,3,4,5,6]:#,4]:
        bc[i] = getBestInClassColor( i )
    lab = { 1:'Confirmed, has $>5\\sigma$ mass', \
            2:'Confirmed, lacking $>5\\sigma$ mass', \
            ## CHANGE BELOW LINES FOR PRE-POST VALIDATION PLOTS
            3:'Unconfirmed TOI', #'Marginal Validation', #'Unconfirmed TOI' }#,pass threshold' 
            4:'Likely False Positive', #'Unconfirmed TOI, fail threshold' }
            5:'Validated Planet', \
            6:'Inconclusive' }
    dy = 0.04
    ytxt0 = 0.86#0.96
    xtxtL = 0.1#0.6
    xtxtR = xtxtL + ( len( lab[2] )+10 )*0.0057#0.0045 # 0.3
    xtxtR2 = xtxtR*1.47
    ytxt = { 1:ytxt0, 2:ytxt0-dy, 3:ytxt0, 4:ytxt0-dy, 5:ytxt0, 6:ytxt0-dy }
    xtxt = { 1:xtxtL, 2:xtxtL, 3:xtxtR, 4:xtxtR, 5:xtxtR2, 6:xtxtR2 }
    ks = [1,2,3,4,5,6]
    for i in range( 6 ):#3 ):#4 ):
        #print( ks[i], xtxt )
        k = ks[i]
...
def applyPreCutsTOIs( z, preCutsFunc, obsSample, limitsRA_hr, limitsDec_deg, onlyPCs=True ):
    ...
    if onlyPCs == True:
        ###print(z.keys())
        ###print(z['planetName'])
        ###print(len(z['planetName']))
        ###blah
        ixsPCs = ( [i[-4:]=='(PC)' for i in z['planetName'][ixs0]] ) #BH 11/2/22
        ###print(len(ixsPCs))
        #ixsPCs = ( [i[-1]=='' for i in z['planetName'][ixs0]] ) #janky fix that assumes z is made up entirely of PCs - won't work for best in class
        ###print(len(ixsPCs))
        #ixsPCs = ( [z['planetName'][ixs0]] )
        #z['planetName'][ixs0][ixsPCs] = z['planetName'][ixs0][ixsPCs][:-4]
        ixs = np.arange( n0 )[ixs0][ixsRA*ixsDec*ixsRp*ixsPCs]
        ###print(ixsPCs)
        #z['planetName'][ixsPCs] = z['planetName'][ixsPCs][:-4]
        ###print(z['planetName'])
        ###blah
    else:
...
def computeTransSignal( RpRs, RpValRE, TeqK, MpValME ):
    ...
    return 4*( RpRs**2. )*( RpValSI*kSI*TeqK )/( mus*GSI*MpValSI )
